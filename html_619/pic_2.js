...
var coloInputObj, widthInputObj, mouseLocationObj;
var lineWidth;
var strokeColor;
...
function init() {
	...
	colorInputObj.onchange = function(e){ // 사용자가 지금 선택한 색으로 strokestyle을 지정
		strokeColor = e.target.value;
		selectedColor.style.backgroundColor = strokeColor;
	}
	
	colorInputObj.onclick = function(e) {// 클릭하면 현재 색으로 stokeColor 변경
		strokeColor = e.target.value;
		selectedColor.style.backgroundColor = strokeColor;
	}
	...
	// 컬러 팔레트 안에 있는 모든 색상 <div>태그에 onclick 리스너를 등록
	var colorObjs = document.getElementsByClassName("color");
	for (var i = 0; i < colorObjs.length ;i++ ) // 사용자가 선택한 색으로 strokeColor 변경
	{
